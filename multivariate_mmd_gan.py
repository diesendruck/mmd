...
parser.add_argument('--model_type', type=str, default='mmd_gan',
                    choices=['mmd_ae', 'mmd_gan', 'kmmd_gan'])
...
def load_data(data_num, percent_train):
    ...
    if data_file:
        ...
        for i in range(data_num):
            ...
            cluster_i = np.random.binomial(1, 0.5)  # NOTE: p=0.
            if cluster_i == 0:
                s = sample_c1()
                data_raw[i] = s
            else:
...
def build_model_kmmd_gan(batch_size, gen_num, data_num, data_test_num, out_dim,
    ...
    # SET UP MMD LOSS.
    #mmd = compute_mmd(enc_x, enc_g, use_tf=True, slim_output=True)  # With encoder vals.
    ...
    return (x, z, z_full, x_test, avg_dist_x_test_to_x, x_precompute, x_test_precompute,
            avg_dist_x_test_to_x_precomputed, g, g_full, kmmd, g_optim, summary_op)
            ...
            if step % log_step == 0:
                ...
                if model_type == 'mmd_ae':
                    ...
                    g_full_ = sess.run(g_full,
                        feed_dict={
                            z_full: get_random_z(data_num, z_dim)})
                    print(('Iter:{}, kmmd_gan = {:.4f}').format(step, kmmd_))

                    verbose = 0
                    if verbose:
                        rd = random_batch_data
                        rg = g_full_[np.random.choice(len(data), 200)]
                        tk, tk_ = compute_kmmd(rd, rg, k_moments=k_moments,
                            kernel=kernel_choice, sigma_list=[sigma])
                        tm, tm_ = compute_mmd(rd, rg, sigma_list=[sigma])
                        print('kmmd: {:.2f}, mmd: {:.2f}'.format(tk, tm))
                        print('max tk_: {:.2f}, max tm_: {:.2f}'.format(
                            np.max(tk_), np.max(tm_)))
                        pdb.set_trace()
