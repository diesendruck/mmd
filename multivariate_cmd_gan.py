...
layers = tf.layers
from mmd_utils import compute_mmd, compute_cmd
...
parser.add_argument('--model_type', type=str, default='mmd_gan',
                    choices=['mmd_ae', 'mmd_gan', 'mmd_gan_simple', 'cmd_gan'])
...
def load_data(data_num, percent_train):
    ...
    if data_file:
        ...
        do_normalization = 1
        if do_normalization:
            data_raw = (data_raw - np.mean(data_raw, axis=0)) / np.std(data_raw, axis=0)
        ...
        for i in range(data_num):
            ...
            cluster_i = np.random.binomial(1, 0.8)  # NOTE: p=0.
            if cluster_i == 0:
                s = sample_c1()
                data_raw[i] = s
            else:
...
def build_model_mmd_ae(batch_size, data_num, data_test_num, gen_num, out_dim, z_dim):
    ...
    if mmd_ae_option == 'ae1':
        ...
        subset_indices = np.random.choice(batch_size, int(batch_size * 0.7),
            replace=False)
...
def build_model_mmd_gan_simple(batch_size, gen_num, data_num, data_test_num,
    ...
    return (x, z, z_full, x_test, avg_dist_x_test_to_x, x_precompute,
            x_test_precompute, avg_dist_x_test_to_x_precomputed, g, g_full, mmd,
            g_optim, summary_op)
...
def build_model_cmd_gan(batch_size, gen_num, data_num, data_test_num, out_dim,
    ...
    # SET UP CMD LOSS.
    cmd = compute_cmd(x, g, k_moments=k_moments, use_tf=True, cmd_a=cmd_a, cmd_b=cmd_b)
    ...
    return (x, z, z_full, x_test, avg_dist_x_test_to_x, x_precompute, x_test_precompute,
            avg_dist_x_test_to_x_precomputed, g, g_full, cmd, g_optim, summary_op)
...
def main():
    ...
    log_dir, checkpoint_dir, plot_dir = prepare_dirs(load_existing)
    save_tag = str(args)
    ...
    if model_type == 'mmd_ae':
        ...
        (x, z, z_full, x_test, avg_dist_x_test_to_x, x_precompute,
         x_test_precompute, avg_dist_x_test_to_x_precomputed, g, g_full, mmd,
         g_optim, summary_op) = build_model_mmd_gan_simple(
             batch_size, gen_num, data_num, data_test_num, out_dim, z_dim)
            ...
            if step % log_step == 0:
                ...
                if model_type == 'mmd_ae':
                    ...
                    print(('Iter: {}, d_loss = {:.4f}, ae_loss = {:.4f}, '
                        'mmd_ae = {:.4f}').format(step, d_loss_, ae_loss_, mmd_))
                    ...
                    g_full_ = sess.run(g_full,
                        feed_dict={
                            z_full: get_random_z(data_num, z_dim)})
                    print(('Iter: {}, mmd_gan = {:.4f}').format(step, mmd_))

                elif model_type == 'cmd_gan':
                    cmd_, summary_result = sess.run(
                        [cmd, summary_op],
                        feed_dict={
                            z: random_batch_z,
                            x: random_batch_data,
                    ...
                    g_full_ = sess.run(g_full,
                        feed_dict={
                            z_full: get_random_z(data_num, z_dim)})
                    print('Iter: {}, cmd_gan = {:.4f}'.format(step, cmd_))

                    c, md, mg = compute_cmd(data, g_full_, k_moments=k_moments,
                        cmd_a=cmd_a, cmd_b=cmd_b)
                    print('  CMD_full: {:.4f}'.format(c))
                    print('  DATA moments: {}'.format(md))
                    print('  GENS moments: {}'.format(mg))
                ...
                if out_dim == 2:
                    ...
                    # Plot univariate CDF, and compare data's with sim's.
                    """
                    d_w, d_p = shapiro(data[:, 0])
                    g_w, g_p = shapiro(g_full_[:, 0])
                    if data_file == '':
                        b_w, b_p = shapiro(both[:, 0])
                    fig, ax = plt.subplots()
                    ax.hist(data[:, 0], bins=data.shape[0], normed=True,
                        cumulative=True, histtype='step',
                        color='gray', alpha=0.8,
                        label='data, SW(w,p)={:.3f},{:.3f}'.format(d_w, d_p))
                    ax.hist(g_full_[:, 0], bins=g_full_.shape[0], normed=True,
                        cumulative=True, histtype='step',
                        color='red', alpha=0.8,
                        label='gen, SW(w,p)={:.3f},{:.3f}'.format(g_w, g_p))
                    if data_file == '':
                        ax.hist(both[:, 0], bins=both.shape[0], normed=True,
                            cumulative=True, histtype='step',
                            color='blue', alpha=0.8,
                            label='both, SW(w,p)={:.3f},{:.3f}'.format(b_w, b_p))
                    plt.legend()
                    plt.savefig(os.path.join(
                        plot_dir, 'cdf_i{}.png'.format(step)))
                    plt.close(fig)
                    """
